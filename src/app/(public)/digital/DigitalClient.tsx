    ...
    <div>
      <section ref={heroRef} className="relative min-h-screen flex items-end bg-secondary overflow-hidden">
        <div className="absolute inset-0">
          <img src="https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=1600&q=80" alt="" className="w-full h-full object-cover" />
              ...
              <div key={project.slug}>
                <Link href={`/portfolio/${project.slug}`} className="group block">
                  <div className="relative aspect-[4/5] overflow-hidden bg-white/5">
                    {project.cover_image ? (<img src={project.cover_image} alt={project.title} className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700" />) : (<div className="absolute inset-0 flex items-center justify-center"><span className="text-white/20 text-sm">Aucune image</span></div>)}
                    ...
                    <div className="absolute top-4 right-4 opacity-0 group-hover:opacity-100 transition-opacity duration-500"><ArrowDownRight className="w-6 h-6 text-white" /></div>
                  </div>
                </Link>
              </div>
            ))}
      ...
      <section className="py-32 lg:py-40 bg-primary relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <p className="text-white/60 max-w-xl mx-auto mb-10 text-lg">{cta.subtitle}</p>
          <Link href={cta.btn_href} className="group inline-flex items-center gap-3 px-8 py-4 border border-white text-white text-sm font-medium tracking-wide hover:bg-white hover:text-primary transition-all duration-300 hover:scale-105">{cta.btn_text} <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" /></Link>
        </div>
      </section>
    </div>
  )
